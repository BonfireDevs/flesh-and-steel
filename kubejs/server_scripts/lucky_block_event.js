 ...
 */
const LUCKY_EVENTS = {
    'lucky:lucky_block': ['gift', 'mob', 'potion'],
    'lucky:elewind_lucky_block': ['cobweb', 'cobweb', 'mob', 'mob', 'boss', 'lucky_block'],
    'lucky:elergy_lucky_block': ['lightning', 'lightning', 'mob', 'mob', 'boss', 'lucky_block'],
    'lucky:eleice_lucky_block': ['snow', 'snow', 'mob', 'mob', 'boss', 'lucky_block'],
    'lucky:elefire_lucky_block': ['lava', 'lava', 'mob', 'mob', 'boss', 'lucky_block'],
    'lucky:elemental_lucky_block': ['cobweb', 'lightning', 'snow', 'lava', 'mob', 'boss', 'lucky_block'],
    'lucky:desert_lucky_block': ['potion', 'mob', 'mob', 'boss', 'lucky_block'],
    'lucky:amongus_lucky_block': ['gift', 'mob', 'mob', 'boss', 'lucky_block'],
    'lucky:summer_lucky_block': ['potion', 'mob', 'mob', 'boss', 'lucky_block'],
    'lucky:fire_lucky_block': ['lava', 'mob', 'boss', 'lucky_block'],
    'lucky:water_lucky_block': ['explosion', 'mob', 'mob', 'boss', 'lucky_block'],
    'lucky:morbius_lucky_block': ['potion', 'mob', 'mob', 'boss', 'lucky_block'],
    'lucky:pink_lucky_block': ['potion', 'mob', 'mob', 'boss', 'lucky_block'],
    'lucky:flesh_and_steel_lucky_block': ['gift', 'lucky_block'],
    'lucky:cheese_lucky_block': ['explosion', 'mob', 'boss'],
    'lucky:blood_lucky_block': ['explosion', 'mob', 'boss'],
    'lucky:cloud_lucky_block': ['explosion', 'mob', 'boss'],
    'lucky:glow_lucky_block': ['explosion', 'mob', 'boss'],
    'lucky:magical_lucky_potion': ['explosion', 'mob', 'boss'],
    'lucky:shadow_lucky_block': ['explosion', 'mob', 'boss'],
    'lucky:void_lucky_block': ['explosion', 'mob', 'boss'],
    'lucky:twilight_lucky_block': ['explosion', 'mob', 'boss'],
    'lucky:strange_lucky_block': ['lightning', 'mob', 'boss'],
}
...
const $TriggerEvents = {
     ...
     */
    lucky_block: (server, player) => {
        server.runCommandSilent(`execute in ${player.level.dimension} run fill ${Math.round(player.x)} ${Math.round(player.y)} ${Math.round(player.z)} ${Math.round(player.x)} ${Math.round(player.y)} ${Math.round(player.z)} ${LUCKY_BLOCKS[Math.floor(Math.random() * LUCKY_BLOCKS.length)]} destroy`)
        server.runCommandSilent(`execute in ${player.level.dimension} run fill ${Math.round(player.x)} ${Math.round(player.y + 1)} ${Math.round(player.z)} ${Math.round(player.x)} ${Math.round(player.y + 1)} ${Math.round(player.z)} minecraft:oak_pressure_plate destroy`)
    },
    /**
     * @param {Internal.MinecraftServer} server 
     * @param {Internal.Player} player 
    ...
    cobweb: (server, player) => {
        ...
        if (difficulty > 100) {
            server.runCommandSilent(`execute in ${player.level.dimension} run fill ${Math.round(player.x) - 1} ${Math.round(player.y)} ${Math.round(player.z) - 1} ${Math.round(player.x) + 1} ${Math.round(player.y + 2)} ${Math.round(player.z) + 1} cobweb destroy`)
        } else if (difficulty > 50) {
            server.runCommandSilent(`execute in ${player.level.dimension} run fill ${Math.round(player.x) - 1} ${Math.round(player.y)} ${Math.round(player.z) - 1} ${Math.round(player.x) + 1} ${Math.round(player.y + 1)} ${Math.round(player.z) + 1} cobweb destroy`)
        } else if (difficulty > 20) {
            server.runCommandSilent(`execute in ${player.level.dimension} run fill ${Math.round(player.x) - 1} ${Math.round(player.y)} ${Math.round(player.z) - 1} ${Math.round(player.x) + 1} ${Math.round(player.y)} ${Math.round(player.z) + 1} cobweb destroy`)
        } else if (difficulty > 5) {
            server.runCommandSilent(`execute in ${player.level.dimension} run fill ${Math.round(player.x)} ${Math.round(player.y + 1)} ${Math.round(player.z)} ${Math.round(player.x)} ${Math.round(player.y)} ${Math.round(player.z)} cobweb destroy`)
        } else {
            server.runCommandSilent(`execute in ${player.level.dimension} run fill ${Math.round(player.x)} ${Math.round(player.y)} ${Math.round(player.z)} ${Math.round(player.x)} ${Math.round(player.y)} ${Math.round(player.z)} cobweb destroy`)
        }
    },
    /**
     * @param {Internal.MinecraftServer} server 
    ...
    position_tp: (server, player) => {
        ...
        if (alivePlayer.size()) {
            ...
            server.runCommandSilent(`execute in ${targetPlayer.level.dimension} run tp @p[name='${player.getName().getString()}'] ${targetPlayer.x} ${targetPlayer.y} ${targetPlayer.z}`)
        } else {
            player.tell(Text.translate('message.kubejs.event.position_tp.random').red())
            server.runCommandSilent(`spreadplayers ${player.x} ${player.z} 0 500 false @p[name='${player.getName().getString()}']`)
        }
    },
    /**
     * @param {Internal.MinecraftServer} server 
    ...
    position_ex: (server, player) => {
        ...
        if (alivePlayer.size()) {
            ...
            server.runCommandSilent(`execute in ${targetPlayer.level.dimension} run tp @p[name='${player.getName().getString()}'] ${targetPlayer.x} ${targetPlayer.y} ${targetPlayer.z}`)
            server.runCommandSilent(`execute in ${position.dimension} run tp @p[name='${targetPlayer.getName().getString()}'] ${position.x} ${position.y} ${position.z}`)
        } else {
            player.tell(Text.translate('message.kubejs.event.position_tp.random').red())
            server.runCommandSilent(`spreadplayers ~ ~ 100 500 false @p[name='${player.getName().getString()}']`)
        }
    },
    /**
     * @param {Internal.MinecraftServer} server 
    ...
    snow: (server, player) => {
        ...
        if (difficulty > 100) {
            server.runCommandSilent(`execute in ${player.level.dimension} run fill ${Math.round(player.x) - 1} ${Math.round(player.y - 1)} ${Math.round(player.z) - 1} ${Math.round(player.x) + 1} ${Math.round(player.y - 1)} ${Math.round(player.z) + 1} powder_snow destroy`)
        } else if (difficulty > 50) {
            server.runCommandSilent(`execute in ${player.level.dimension} run fill ${Math.round(player.x)} ${Math.round(player.y - 1)} ${Math.round(player.z)} ${Math.round(player.x)} ${Math.round(player.y - 1)} ${Math.round(player.z)} powder_snow destroy`)
        } else if (difficulty > 20) {
            server.runCommandSilent(`execute in ${player.level.dimension} run fill ${Math.round(player.x)} ${Math.round(player.y - 1)} ${Math.round(player.z)} ${Math.round(player.x)} ${Math.round(player.y - 1)} ${Math.round(player.z)} powder_snow destroy`)
        } else if (difficulty > 5) {
            server.runCommandSilent(`execute in ${player.level.dimension} run fill ${Math.round(player.x)} ${Math.round(player.y - 1)} ${Math.round(player.z)} ${Math.round(player.x)} ${Math.round(player.y - 1)} ${Math.round(player.z)} powder_snow destroy`)
    ...
    lava: (server, player) => {
        ...
        if (difficulty > 100) {
            server.runCommandSilent(`execute in ${player.level.dimension} run fill ${Math.round(player.x) - 1} ${Math.round(player.y - 1)} ${Math.round(player.z) - 1} ${Math.round(player.x) + 1} ${Math.round(player.y - 1)} ${Math.round(player.z) + 1} lava destroy`)
        } else if (difficulty > 50) {
            server.runCommandSilent(`execute in ${player.level.dimension} run fill ${Math.round(player.x)} ${Math.round(player.y - 1)} ${Math.round(player.z)} ${Math.round(player.x)} ${Math.round(player.y - 1)} ${Math.round(player.z)} lava destroy`)
        } else if (difficulty > 20) {
            server.runCommandSilent(`execute in ${player.level.dimension} run fill ${Math.round(player.x)} ${Math.round(player.y - 1)} ${Math.round(player.z)} ${Math.round(player.x)} ${Math.round(player.y - 1)} ${Math.round(player.z)} lava destroy`)
        } else if (difficulty > 5) {
            server.runCommandSilent(`execute in ${player.level.dimension} run fill ${Math.round(player.x)} ${Math.round(player.y - 1)} ${Math.round(player.z)} ${Math.round(player.x)} ${Math.round(player.y - 1)} ${Math.round(player.z)} lava destroy`)
